...
class ShopController extends Controller
{
    ...
    public function ShopView(Request $request){
      ...
      //    }
        ...
        return view('frontend.shop.shop_view',compact('products','categories','brands','priceProds','test'));
    }
...
// dd($request->all());
         ...
         $sort= $request->sort;
         // return response()->json(['min_price' => $min_price,'paginate' => $paginate]);	
         if (!empty($categories)) {
               $catIds = Category::select('id')->whereIn('category_slug_en',$categories)->pluck('id')->toArray();
               $priceProds = $productQ->whereIn('category_id',$catIds)->get();
               $products = $productQ->whereIn('category_id',$catIds)->paginate($paginate);
         } 
         if (!empty($brands)) {
            $brandIds = Brand::select('id')->whereIn('brand_slug_en',$brands)->pluck('id')->toArray();
            $products = $productQ->whereIn('brand_id',$brandIds)->paginate($paginate);
            $priceProds = $productQ->whereIn('brand_id',$brandIds)->get();
         }
         $priceProds = $productQ->where('status',1)->whereBetween('selling_price', [$min_price, $max_price])->get();
         $products = $productQ->where('status',1)->whereBetween('selling_price', [$min_price, $max_price])->orderBy('id','DESC')->paginate($paginate);
         $productlist = view('frontend.product.products',compact('products','priceProds'))->render();
         return response()->json(['productlist' => $productlist]);	
         // $list_view = view('frontend.product.list_view_product',compact('products','priceProds'))->render();
         // $list_view = view('frontend.product.grid_view_product',compact('products','priceProds'))->render();
         // return response()->json(['grid_view' => $grid_view,'list_view' => $list_view]);	
    }
}
